...
public class KYCPresentAddressActivity extends Fragment {
    private final String TAG = getClass().getName() + " Atiar - ";

    public KYCPresentAddressActivity() {
    }
    ...
    @BindView(R.id.radioMembersObosthan)     RadioGroup _membersObosthan;
    @BindView(R.id.ownHouse)                 RadioButton _radioOwnHouse;
    @BindView(R.id.rentedHouse)              RadioButton _radioRentedHouse;
    @BindView(R.id.ed_elakay_koto_bochor_achen) EditText _elakayKotoBochorAchen;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.activity_present_address, container, false);
    ...
    private void setkycPresentAddress(KYCPresentAddressBox data) {
        ...
        data.setRoad_no(_roadName.getText().toString());
        ...
        switch (_membersObosthan.getCheckedRadioButtonId()){
            case R.id.ownHouse:
                isowner = 1;
                break;
            case R.id.rentedHouse:
                isowner = 0;
                break;
        }
        data.setIs_owner(isowner);

        data.setDuration_liveshere(Integer.parseInt(_elakayKotoBochorAchen.getText().toString().trim()));

    }

    private void saveDataToLocalStorage(){

        KYCPresentAddressBox kycPresentAddress = _objectBoxManager.GetKYCPresentAddressBox(member.getBRANCH_ID(), member.getMEMBER_ID());

        if (kycPresentAddress == null) {
            KYCPresentAddressBox newData = new KYCPresentAddressBox();
            setkycPresentAddress(newData);
            _objectBoxManager.SaveKYCPresentAddressBox(newData);
        } else {
            setkycPresentAddress(kycPresentAddress);
            _objectBoxManager.SaveKYCPresentAddressBox(kycPresentAddress);
        }
    ...
    private void loadFormIfDataAvailable(){
        ...
        KYCPresentAddressBox data = _objectBoxManager.GetKYCPresentAddressBox(member.getBRANCH_ID(), member.getMEMBER_ID());
        if (data != null) {
            ...
            _elakayKotoBochorAchen.setText(data.getDuration_liveshere() + "");
            _membersObosthan.check((data.getIs_owner().equals("1")) ? _radioRentedHouse.getId() : _radioOwnHouse.getId());
        }
